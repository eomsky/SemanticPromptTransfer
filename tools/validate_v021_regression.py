...
def main(v020_root: str, v021_root: str, master_path: str, output_path: str) -> int:
    old = snapshot(Path(v020_root).resolve(), Path(master_path).resolve())
    new = snapshot(Path(v021_root).resolve(), Path(master_path).resolve())
    report = {
        "baseline": "v0.20.0",
        "candidate": "v0.21.0",
