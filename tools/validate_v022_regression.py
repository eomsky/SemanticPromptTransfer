...
def snapshot(package_root: Path, master_path: Path) -> dict[str, object]:
    ...
    output = subprocess.check_output(
        [sys.executable, "-c", SNAPSHOT_CODE, str(master_path)], env=env, text=True
    )
    return json.loads(output)


def main(v021_root: str, v022_root: str, master_path: str, output_path: str) -> int:
    old = snapshot(Path(v021_root).resolve(), Path(master_path).resolve())
    new = snapshot(Path(v022_root).resolve(), Path(master_path).resolve())
    report = {
        "baseline": "v0.21.0",
        "candidate": "v0.22.0",
