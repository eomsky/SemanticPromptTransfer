# SemanticPromptTransfer v0.26.2 Colab 운영
...
├── runtime-assets/v0.26.2/SemanticPromptTransfer_v0.26.2_COLAB_ASSETS.json
└── versions/v0.26.2/semantic_prompt_transfer-0.26.2-py3-none-any.whl
```

## 실행

1. 새 Colab 런타임에서 `SemanticPromptTransfer_v0.26.2_COLAB_LAUNCHER.ipynb`를 연다.
2. A100 GPU를 선택하고 모든 셀을 위에서 아래로 실행한다.
3. `E5 dependency stack ready · vLLM environment isolated`를 확인한다.
4. `loading native vLLM`과 `vLLM ready`를 확인한다.
...
엔진이 실패하면 예외에 `/content/spt_bootstrap_v0262/vllm.log` 경로와
`EngineCore failed to start` 직전부터의 최초 원인 스택이 함께 표시된다.
마지막 공통 `Engine core initialization failed` 문구만으로 원인을 판단하지 않는다.
