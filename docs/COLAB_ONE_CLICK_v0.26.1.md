# SemanticPromptTransfer v0.26.1 Colab 운영
...
├── runtime-assets/v0.26.1/SemanticPromptTransfer_v0.26.1_COLAB_ASSETS.json
└── versions/v0.26.1/semantic_prompt_transfer-0.26.1-py3-none-any.whl
```

매니페스트는 wheel의 크기와 SHA-256을 검사한다. v0.26.1은 GPU E5를
Hugging Face에서 로드하므로 이전 ONNX 압축 자산은 필요하지 않다.

## 실행

1. `SemanticPromptTransfer_v0.26.1_COLAB_LAUNCHER.ipynb`를 Colab에서 연다.
2. 런타임 유형을 A100 GPU로 선택한다.
3. 설정 셀, FEW SHOT 1~3 셀, 운영 개시 셀을 위에서 아래로 실행한다.
4. `vLLM ready`와 `ready: package=0.26.1` 로그를 확인한다.
...
vLLM과 CUDA/PyTorch 의존성은 `stage_root/vllm-env` 전용 `uv` 환경에 설치한다. 현재 Colab 커널의 NumPy·SciPy·transformers를 변경하지 않으며, E5 의존성 검사를 대형 모델 다운로드 전에 완료한다.
